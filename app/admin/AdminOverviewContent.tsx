...
type Overview = FunctionReturnType<typeof api.admin.overview>
type ActivityRow = Overview['recentActivity'][number]

export default function AdminOverviewContent({
  preloaded,
}: {
  preloaded: Preloaded<typeof api.admin.overview>
}) {
  const { user } = useAuth()
  const { stats, recentActivity: activities } = usePreloadedQuery(preloaded)
    ...
    <div className="space-y-6">
      ...
      >
        ...
        <StatsCard
          ...
          value={stats.newHelpInterests.toString()}
          icon={Users}
          color="blue"
        />
        <StatsCard
        ...
        <StatsCard
          ...
          value={stats.featuredStudents.toString()}
          icon={Star}
          color="primary"
        />
      </motion.div>
...
function ActivityCard({
  ...
  const getActivityDescription = () => {
    if (activity.actionType === 'status_change') {
      const oldStatus = (activity.oldValue as { status?: string })?.status || 'unknown'
      const newStatus = (activity.newValue as { status?: string })?.status || 'unknown'
      return `Changed status from "${formatStatus(oldStatus)}" to "${formatStatus(newStatus)}"`
    }
    if (activity.actionType === 'notes_update') {
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs text-gray-500 mt-1">
          {activity.entityType === 'application' && (
            <Link
              href={`/admin/scholarship-applications/${activity.entityId}`}
      ...
      <div className="text-right shrink-0">
        ...
        <p className="text-xs text-gray-400">
          {new Date(activity._creationTime!).toLocaleDateString('en-IN', {
            day: 'numeric',
            month: 'short',
            hour: '2-digit',
            minute: '2-digit',
