...
// Unified shape for showing both application kinds in one list.
...
function toUnified(app: Summary['applications'][number]): UnifiedApplication {
  return {
    id: app._id,
    displayId: app.applicationId,
    type: 'scholarship',
    typeLabel: app.applicationType === 'first-year' ? '1st Year Scholarship' : '2nd Year Renewal Scholarship',
    status: app.status,
    createdAt: app._creationTime,
    detailHref: `/dashboard/applications/${app._id}`,
  }
}

function spotlightToUnified(
  app: Summary['spotlightApplications'][number],
): UnifiedApplication {
  return {
    id: app._id,
    displayId: app.spotlightId,
    type: 'spotlight',
    typeLabel: 'Spotlight Application',
    status: app.status,
    createdAt: app._creationTime,
    detailHref: `/dashboard/spotlight/${app._id}`,
  }
}
...
export default function ApplicationsContent({
  ...
  // Derived from the query result rather than copied into state by an effect.
  // The old version mirrored the fetch into `allApplications` and then mirrored
  // that into `filteredApplications` from a second effect — two renders behind
  // the data, and the pattern eslint's react-hooks/set-state-in-effect flags.
  const allApplications = useMemo(() => {
    const unified = [
      ...(data?.applications ?? []).map(toUnified),
      ...(data?.spotlightApplications ?? []).map(spotlightToUnified),
    ]
  ...
  const filteredApplications = useMemo(() => {
    ...
    return filtered
  }, [allApplications, searchQuery, statusFilter, typeFilter])
