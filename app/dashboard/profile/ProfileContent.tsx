...
// Stays a Client Component: this is a form, which the rendering plan keeps in
// the interactive tier. It reads through AuthContext, which is already backed
// by the Convex users.me query — only the write was still pointed at a dead
// API route.
//
// formData keeps snake_case keys because AuthContext still hands back the
// legacy Student shape via toLegacyStudent(). That shim is Phase 3's to delete;
// mapping to the mutation's camelCase happens at the call site below.
export default function ProfileContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // updateProfile only patches the keys it receives, so sending undefined
      // for a cleared field leaves the stored value alone rather than blanking
      // it. gender is a union in the schema — an empty select must be omitted,
      // not sent as "".
      await updateProfile({
      ...
      // No refetch: AuthContext's users.me query is a live subscription, so the
      // new values arrive on their own. This is what made refreshStudent() a
      // no-op after the Convex cutover.
      setIsEditing(false)
    } catch (err) {
