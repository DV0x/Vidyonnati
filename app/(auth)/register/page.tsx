...
import { Loader2, UserPlus, Eye, EyeOff, Check, ShieldCheck } from "lucide-react"
...
function RegisterContent() {
  ...
  // Clerk verifies email with a code, not a magic link. The old Supabase flow
  // sent users away to /login?registered=true to wait for an email; this keeps
  // them in place and signs them straight in once verified.
  const [needsCode, setNeedsCode] = useState(false)
  const [code, setCode] = useState("")

  // Client-side UX only — Clerk enforces its own password policy server-side.
  ...
  const handleEmailRegister = async (e: React.FormEvent) => {
    ...
    try {
      // Clerk stores first/last separately and composes `name` from them,
      // which is what the Convex profile helper reads off the JWT identity.
      ...
      if (error) {
        // signUp.isTransferable means the email already belongs to a user and
        // this should become a sign-in instead.
      ...
      await signUp.finalize({
        navigate: ({ session, decorateUrl }) => {
          if (session?.currentTask) {
            console.warn("Sign-up blocked by pending task:", session.currentTask)
            setIsLoading(false)
            return
          }
  ...
  const handleGoogleRegister = async () => {
    ...
    if (error) {
      toast({
        ...
        description: "Could not start Google sign-up. Please try again.",
        variant: "destructive",
      })
        ...
        >
          ...
          <div className="text-center mb-6">
            ...
            <h1 className="text-2xl sm:text-3xl font-bold text-gray-900">
              {needsCode ? "Verify Your Email" : "Create Account"}
            </h1>
          ...
          <div className="bg-white/80 backdrop-blur-xl rounded-2xl border border-white/50 shadow-xl p-5 sm:p-6">
            ...
            {/* Required by Clerk — bot sign-up protection is enabled by default
                and renders its challenge into this element. Removing it breaks
                sign-up with an opaque error. */}
