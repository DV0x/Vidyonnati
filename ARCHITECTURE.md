...
- `student_id` (FK), `status`: pending | under_review | approved | rejected
...
- Token refresh events do NOT set `isLoading` (prevents UI flashing)
...
|--------|------|-----|---------|
| Browser | `lib/supabase/client.ts` | ANON_KEY | Client components |
...
| `/dashboard` | `app/dashboard/page.tsx` | Student applications overview |
| `/dashboard/applications` | `app/dashboard/applications/page.tsx` | All applications list |
...
| POST | `/api/student/applications` | Create scholarship application |
| GET/POST | `/api/student/spotlight` | List/create spotlight apps |
| GET | `/api/student/spotlight/[id]` | Single spotlight app detail |
...
SpotlightStatus    = 'pending' | 'under_review' | 'approved' | 'rejected'
...
3. **Data fetching via API routes**: All dashboard pages fetch data through `/api/` routes (server-side Supabase client with cookies) rather than browser Supabase client directly. This avoids browser client auth timing issues.
...
9. **Draft persistence**: localStorage keyed by application type with 24hr TTL, excludes file data
